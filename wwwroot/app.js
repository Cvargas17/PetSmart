...
async function saveProduct(event) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return showAlert(data?.error || 'Error guardando el producto.', 'error');
    }
...
async function handleTableClick(event) {
  ...
  if (action === 'delete') {
    ...
    if (!response.ok) {
      const data = await response.json();
      return showAlert(data?.error || 'Error eliminando el producto.', 'error');
    }
  ...
  if (action === 'notify') {
    ...
    const data = await response.json();
    if (!response.ok) {
      return showAlert(data?.error || 'Error enviando la notificación.', 'error');
    }
